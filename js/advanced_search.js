...
function addSearch(group, term, field)
{
    ...
    var newSearch = '<div class="row">';

    // Label
    newSearch += '<div class="form-label columns large-3"><label class="';
    if (groupSearches[group] > 0) {
        newSearch += 'hide ';
    }
    newSearch += 'right inline" for="search_lookfor' + group + '_' + groupSearches[group] + '">' + searchLabel + ':</label>&nbsp;</div>';

    // Terms
    newSearch += '<div class="terms columns large-5"><input type="text" id="search_lookfor' + group + '_' + groupSearches[group] + '" name="lookfor' + group + '[]" size="50" value="' + jsEntityEncode(term) + '"/></div>';

    // Field
    newSearch += '<div class="field columns large-4"><div class="row"><div class="columns large-2"><label class="right inline" for="search_type' + group + '_' + groupSearches[group] + '">' + searchFieldLabel + '</label></div>';
    newSearch += '<div class="field columns large-10"><select id="search_type' + group + '_' + groupSearches[group] + '" name="type' + group + '[]">';
...
function addGroup(firstTerm, firstField, join)
{
    ...
    var newGroup = '<div id="group' + nextGroupNumber + '" class="panel group group' + (nextGroupNumber % 2) + '">';
    newGroup += '<div class="groupSearchDetails">';

    // Boolean operator drop-down
    newGroup += '<div class="join right"><label for="search_bool' + nextGroupNumber + '">' + searchMatch + ':</label> ';
    ...
    newGroup += '<div id="group' + nextGroupNumber + 'SearchHolder" class="groupSearchHolder clearfix"></div>';

    // Add search term link
    newGroup += '<div class="addSearch clearfix"><a href="#" class="add right" id="add_search_link_' + nextGroupNumber + '" onclick="addSearchJS(this); return false;">' + addSearchString + '</a></div>';
